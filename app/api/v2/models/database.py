'''Module containing definitions for the database'''
import os
import psycopg2
...
class DatabaseSetup:
    '''Definitions for setting up the database'''
    @classmethod
    def initialize_db(cls):
        ...
        queries = DatabaseSetup.create_table_queries()
    ...
    def destroy(cls):
        ...
        connection = psycopg2.connect(database_url)
        cursor = connection.cursor()
        queries = DatabaseSetup.drop_table_queries()
        for query in queries:
            cursor.execute(query)
        connection.commit()
